...
try {
     ...
     $mail->Username = 'ajudatioficial@gmail.com'; 						// Usuário do servidor SMTP (endereço de email)
     $mail->Password = 'P4$$w0rd2'; 						// Senha do servidor SMTP (senha do email usado)
 
     //Define o remetente
     // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=    
     $mail->SetFrom utf8_decode('contato@guimaraespinturas.com.br', 'Contato WebSite'); 	//E-mail do dominio (Obrigatório)
     $mail->AddReplyTo utf8_decode($email, $nome); 							//Seu e-mail
     $mail->Subject = utf8_decode('[Contato Web] Guimarães Pintura');						//Assunto do e-mail
 
     //Define os destinatário(s)
     //=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
     $mail->AddAddress utf8_decode('duchoa@cpfl.com.br', 'Guimarães Pintura'); // E-mail Principal
